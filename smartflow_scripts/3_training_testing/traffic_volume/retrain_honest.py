...
# something.
# Which evaluation arm this run produces. Both arms live side by side in
# gold.*, keyed on split_label, and the dashboard serves 80/20. This used to be
# a bare N_ORIGINS = 10 (the 90/10 arm) that wrote NO split_label and TRUNCATED
# the table first - so one re-run would have deleted the 80/20 results and left
# the volume panel, which filters on split_label, completely empty.
#   SPLIT=90_10 python retrain_honest.py   -> the 90/10 arm
SPLIT = os.environ.get("SPLIT", "80_20")
N_ORIGINS = {"80_20": 21, "90_10": 10}[SPLIT]
...
FUTURE_DAYS = 90   # matches the dashboard's 3-month option; extend_future_volume.py
                   # measures what each stretch of it is actually worth
...
for oi, cut in enumerate(origins, 1):
    ...
    if len(future) < HORIZON:
        break
    # Weather over the forecast window is day-of-year climatology from the
    # TRAINING slice only. Passing `future` straight through handed Prophet,
    # SARIMAX and LSTM the OBSERVED weather for the days they were forecasting -
    # a leak. The emissions trainer had the same bug (it cost 0.12pp WMAPE).
    future_known = future.copy()
    _clim = train.assign(_k=train.ds.dt.dayofyear).groupby("_k")[WEATHER_COLS].mean()
    for _c in WEATHER_COLS:
        future_known[_c] = [float(_clim[_c].get(_d.dayofyear, train[_c].mean())) for _d in future.ds]
...
conn = psycopg2.connect(POSTGRES_URL)
cur = conn.cursor()
# Only this arm. TRUNCATE here used to delete the other arm too.
cur.execute("DELETE FROM gold.ml_predictive_volume WHERE split_label = %s", (SPLIT,))
...
def put(d, actual, p, holdout, future):
    cur.execute(
        ...
        (d["ds"].date(), int(actual) if actual is not None else None,
         ...
         p("Prophet_nw"), p("SARIMAX_nw"), p("LSTM_nw"), SPLIT),
    )
