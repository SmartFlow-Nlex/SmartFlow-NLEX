...
pool.query("SELECT column_name FROM information_schema.columns WHERE table_name = 'hourly_weather'").then(r => {
  console.log('hourly_weather columns:', r.rows.map(c => c.column_name));
  pool.end();
}).catch(console.error);
